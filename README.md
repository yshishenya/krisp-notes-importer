...
Плагин предлагает гибкую настройку через меню Obsidian: **Настройки → Community Plugins → Krisp Notes Importer**.

Основные группы настроек:

- **📊 Текущий Статус**: Отображает статус сервиса отслеживания.
- **🌐 Язык интерфейса**: Выбор языка (Английский/Русский).
- **📂 Ядро: Отслеживание и Автоматизация**:
  - Путь к папке Krisp для мониторинга.
  - Включение/выключение автоматического отслеживания.
- **🗄️ Хранилище Obsidian**:
  - Папка для создаваемых заметок.
  - Папка для аудиофайлов.
- **🎨 Внешний вид и Именование**:
  - Шаблоны для имен заметок и аудиофайлов (например, `{{YYYY}}-{{MM}}-{{DD}}_{{HHMM}}_{{meetingTitle}}`).
  - Полностью настраиваемый шаблон содержимого для Markdown-заметок.
- **⚙️ Поведение импорта**:
  - Стратегия обработки дубликатов (пропустить, перезаписать, создать с суффиксом).
  - Опция открытия заметки после импорта.
  - Опция удаления ZIP-архива после импорта.
- **🛠️ Ручные операции и Диагностика**:
  - Тестовый импорт одного файла.
  - Массовая обработка всех файлов в отслеживаемой папке.
  - Просмотр логов работы плагина.
  - Сброс всех настроек к значениям по умолчанию.
- **ℹ️ О плагине**:
  - Информация о версии, разработчике, ссылки на GitHub.
...
**Статус: Активная разработка. Последнее обновление: 2025-05-30.**

**Последняя версия:** [v3.3.12](https://github.com/your-username/krisp-notes-importer/releases/tag/v3.3.12) (2025-05-30)
